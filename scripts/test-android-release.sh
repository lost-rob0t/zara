...
store_apk="$repo_root/android/apps/zara-store/build/outputs/apk/debug/zara-store-debug.apk"
test -f "$apk"
test -f "$store_apk"
...
store_apk_sha256="$(sha256sum "$store_apk" | awk '{print $1}')"
cat >"$manifest" <<EOF
schema=1
source_sha=$source_sha
apk=android/app/build/outputs/apk/debug/app-debug.apk
apk_sha256=$apk_sha256
store_apk=android/apps/zara-store/build/outputs/apk/debug/zara-store-debug.apk
store_apk_sha256=$store_apk_sha256
...
deterministic.store_apk_build=PASS
deterministic.store_apk_secret_scan=PASS
...
echo "android release gate ok: $apk $store_apk"
