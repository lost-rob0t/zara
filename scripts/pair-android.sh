...
Recovery:
  ...
  3. Reset the local daemon with: scripts/pair-android.sh --reset-adb
  4. QR-capable device: scripts/pair-android.sh --qr
  5. Pairing-code fallback: scripts/pair-android.sh HOST:PAIR_PORT HOST:ADB_PORT

You can inspect ADB/mDNS state with:
  scripts/pair-android.sh --diagnose
EOF
}
...
if [[ -z "$connect_endpoint" ]]; then
  ...
  echo "  scripts/pair-android.sh --connect HOST:ADB_PORT"
  exit 0
fi
...
echo "Install the Android app with: ./gradlew :android:installDebug"
