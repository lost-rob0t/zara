...
# legacy symbolic-owner claim, restart cancellation fencing, and the actual
# native pure-symbolic dialogue runtime on the same emulator used for acceptance.
...
# land. AndroidPureSymbolicConversationInstrumentedTest then runs a real Trealla
# clarification -> follow-up -> reopen -> social continuation sequence and
# asserts provider/model accounting remains exactly zero throughout.
set +e
