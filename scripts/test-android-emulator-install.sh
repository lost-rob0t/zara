...
# legacy symbolic-owner claim, restart cancellation fencing, native Result
# binding/context persistence boundaries, project-switch stale-generation fence,
# verified-effect restart/ABA evidence, and the actual native pure-symbolic
# dialogue runtime on the same emulator used for acceptance. These classes are
# named explicitly, so every acceptance class below must stay in this filter
# rather than merely compiling in androidTest.
set +e
