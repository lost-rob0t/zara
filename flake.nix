    ...
    let
      ...
      mkSystem = system:
        ...
        in
        {
          ...
          # Single source of truth for the Android toolchain is android/flake.nix,
          # the shell CI enters via `nix develop ./android`. Project the exact
          # same environment here through the path input (own lock, no follows)
          # instead of a drifting second copy that lacked the NDK and Trealla
          # pins (#1267). Update with `nix flake update android` when the
          # toolchain flake changes.
          devShells.android = android.devShells.${system}.default;
