...
% Android-packaged portable semantic term core for ZARA-SEMANTIC/1.
% This module intentionally owns no I/O, networking, provider selection,
% dynamic goal dispatch, or server authority. Utterance parsing remains the
% shared semantic-corpus owner until the later #172 Trealla parity slice.
...
% ZARA-SEMANTIC/1 compares a frame's slots as a set. Canonicalize slot order
% without changing values or provenance so SWI/Trealla outputs can be compared
% by the later cross-runtime runner.
normalize_frames([], []).
