...
/**
 * Authority boundary for user-owned Prolog.
 *
 * Query syntax filtering is not enough: a harmless-looking query can call a user rule whose body
 * reaches an effectful builtin. This policy validates the executable portions of workspace sources
 * before they are ever consulted by Trealla, and applies the same deny-set to ad-hoc queries.
 */
object PrologAuthorityPolicy {
    ...
    fun validate(document: PrologDocument): List<PrologDiagnostic> {
        ...
        diagnostics += PrologSchemaValidator.validate(document)
    ...
    fun requireSafeWorkspace(sources: List<PrologSource>) {
        val violations = sources.flatMap { source ->
            validate(source).map { diagnostic -> "${source.name}:${diagnostic.line}: ${diagnostic.message}" }
        }
    ...
    private fun validateDirective(clause: PrologClause): PrologDiagnostic? {
        ...
        return PrologDiagnostic(
            clause.line,
            "Only zara_schema/3 and validated op/3 directives are allowed in the private workspace",
        )
    }
