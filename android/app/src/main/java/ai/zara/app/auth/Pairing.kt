...
class PairingClient(
    ...
    internal fun pairReserved(
        ...
        registerSocket(pairingGeneration, socket)
        try {
            socket.connect(
            ...
            if (isCancelled(pairingGeneration)) {
                throw PairingException("pairing was cancelled", error)
            }
            throw PairingException("Zara pairing failed", error)
        } finally {
            clearSocket(pairingGeneration, socket)
            closeQuietly(socket)
        }
    }

    fun cancelCurrent() {
        val socket = synchronized(lifecycleLock) {
            if (closed) return
            generation += 1
    ...
    private fun registerSocket(pairingGeneration: Long, socket: Socket) {
        synchronized(lifecycleLock) {
            requireCurrentLocked(pairingGeneration)
            check(activeSocket == null) { "pairing socket is already active" }
            activeSocket = socket
        }
    }

    private fun clearSocket(pairingGeneration: Long, socket: Socket) {
        synchronized(lifecycleLock) {
            if (generation == pairingGeneration && activeSocket === socket) {
                activeSocket = null
            }
        }
    }

    private fun requireCurrent(pairingGeneration: Long) {
        synchronized(lifecycleLock) {
            requireCurrentLocked(pairingGeneration)
        }
    }

    private fun requireCurrentLocked(pairingGeneration: Long) {
        if (closed || generation != pairingGeneration) {
            throw PairingException("pairing was cancelled")
        }
    }

    private fun isCancelled(pairingGeneration: Long): Boolean = synchronized(lifecycleLock) {
        closed || generation != pairingGeneration
    }
    ...
    private fun closeQuietly(socket: Socket?) {
        try {
            ...
            // Cancellation is best-effort at the socket layer; generation fencing remains authoritative.
        }
    }
