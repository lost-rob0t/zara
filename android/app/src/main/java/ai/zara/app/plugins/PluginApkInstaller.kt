...
internal class PluginApkInstaller(private val context: Context) {
    ...
    fun requestInstall() {
        ...
        submit(setOf(PluginInstallPhase.REVIEW, PluginInstallPhase.PERMISSION_REQUIRED, PluginInstallPhase.FAILED)) { previous ->
            ...
            if (!context.packageManager.canRequestPackageInstalls()) {
                publish(PluginInstallState(
                    PluginInstallPhase.PERMISSION_REQUIRED, candidate,
            ...
            var committed = false
            try {
                publish(PluginInstallState(PluginInstallPhase.VERIFYING, candidate, sessionId, nonce), durable = true)
                ...
                publish(PluginInstallState(
                    PluginInstallPhase.INSTALLING, candidate, sessionId, nonce,
    ...
    fun permissionResult() {
        synchronized(lock) {
            ...
            publish(current.copy(
                ...
                message = if (granted) "Permission granted. Tap Install plugin to continue."
                    else "Install permission was not granted. You can retry or choose another APK.",
            ))
        }
    }
    ...
    fun receiveResult(intent: Intent): Intent? = synchronized(lock) {
        ...
        when (intent.getIntExtra(PackageInstaller.EXTRA_STATUS, PackageInstaller.STATUS_FAILURE)) {
            ...
            PackageInstaller.STATUS_SUCCESS -> {
                current.candidate?.let { cachedFile(it).delete() }
                publish(current.copy(
                    phase = PluginInstallPhase.INSTALLED, sessionId = -1, nonce = "",
                    message = "Android installed the APK. Plugin trust and capabilities were not enabled.",
                ))
                null
            }
            else -> {
    ...
    fun confirmationFailed(intent: Intent) {
        synchronized(lock) {
            ...
            runCatching { context.packageManager.packageInstaller.abandonSession(current.sessionId) }
            publish(current.copy(
                phase = PluginInstallPhase.FAILED, sessionId = -1, nonce = "",
    ...
    private fun matchesCurrentSession(intent: Intent): Boolean =
        current.phase == PluginInstallPhase.INSTALLING &&
            ...
            PluginApkSecurity.matchesCallback(
                current.sessionId, current.nonce,
    ...
    private fun inspect(apk: File, checksum: String): PluginApkCandidate {
        ...
        ) ?: throw IllegalArgumentException("Select a complete Android APK, not a ZIP, split APK bundle, or desktop plugin.")
        ...
        return PluginApkCandidate(
            apk.name, info.packageName, (info.versionName ?: info.longVersionCode.toString()).take(120),
            checksum, certificates,
        )
    }

    private fun cachedFile(candidate: PluginApkCandidate): File {
        require(candidate.fileName.matches(Regex("plugin-[A-Za-z0-9-]+\\.apk"))) { "Invalid APK cache identity." }
                    ...
                    synchronized(lock) {
                        if (current.phase != PluginInstallPhase.INSTALLED) {
                            val message = when (failure) {
                                is IllegalArgumentException, is IllegalStateException -> failure.message?.take(240)
    ...
    private fun publish(value: PluginInstallState, durable: Boolean = false) {
        synchronized(lock) {
            val json = JSONObject()
                .put("phase", value.phase.name).put("session", value.sessionId)
                .put("nonce", value.nonce).put("message", value.message ?: JSONObject.NULL)
            value.candidate?.let { candidate ->
                json.put("candidate", JSONObject()
                    .put("file", candidate.fileName).put("package", candidate.packageName)
                    .put("version", candidate.version).put("sha256", candidate.sha256)
    ...
    private fun restore(): PluginInstallState = try {
        ...
        if (raw == null || raw.length > 16_384) {
            ...
            val candidate = json.optJSONObject("candidate")?.let { item ->
                ...
                PluginApkCandidate(
                    item.getString("file"), item.getString("package"), item.getString("version").take(120),
            ...
            if (phase == PluginInstallPhase.VERIFYING) {
                ...
                PluginInstallState(PluginInstallPhase.FAILED, candidate,
                    message = "APK preparation was interrupted. Select the APK again or retry.")
            } else {
                PluginInstallState(phase, candidate, session, json.optString("nonce"),
                    if (json.isNull("message")) null else json.optString("message").take(240))
            }
        }
    } catch (_: Exception) {
        PluginInstallState(PluginInstallPhase.FAILED, message = "Installer state could not be restored. Select the APK again.")
    }
    ...
    companion object {
        ...
        private val editablePhases = setOf(
            PluginInstallPhase.IDLE, PluginInstallPhase.REVIEW, PluginInstallPhase.PERMISSION_REQUIRED,
            PluginInstallPhase.INSTALLED, PluginInstallPhase.FAILED,
        )
    }
}
