...
class CloudModelCoordinator(
    ...
    fun generate(
        ...
        actor.execute {
            ...
            try {
                ...
                if (snapshot.config.provider.requiresApiKey && apiKey.isNullOrBlank()) {
                    throw CloudModelException(
                        CloudModelFailureReason.AUTHENTICATION,
                        "Model-server API key is not configured",
                    )
                }
    ...
    fun discoverModels(): CompletableFuture<List<String>> {
        val refreshed = try {
            reloadConfig()
        } catch (error: Throwable) {
            return CompletableFuture.failedFuture(error)
        ...
        actor.execute {
            try {
                val apiKey = apiKeyStore.load()
                if (refreshed.config.provider.requiresApiKey && apiKey.isNullOrBlank()) {
                    throw CloudModelException(
                        CloudModelFailureReason.AUTHENTICATION,
                        "Model-server API key is not configured",
                    )
