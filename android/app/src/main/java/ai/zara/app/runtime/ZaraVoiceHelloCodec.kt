...
data class AudioOutputFormat(
    ...
    companion object {
        fun pcmS16leMono(sampleRate: Int): AudioOutputFormat =
            AudioOutputFormat("pcm_s16le", sampleRate, 1)
    }
}

data class VoiceHelloOk(
    val replyTo: String,
...
object ZaraVoiceHelloCodec {
    ...
    fun encodeHello(
        requestId: String,
        timestampNs: Long,
        audioOutputFormats: List<AudioOutputFormat>,
    ): List<ByteArray> {
        ...
        val envelope =
            "{\"body\":{\"audio_output_formats\":$formats,\"versions\":[1]}," +
    ...
    fun decodeHelloOk(frames: List<ByteArray>): VoiceHelloOk {
        if (frames.size != 2 || !frames[0].contentEquals(marker)) {
            throw ZaraWireException("invalid ZARA/1 voice hello frame")
        }
        ...
        if (requireString(envelope, "type", 64) != "hello.ok") {
        ...
        return VoiceHelloOk(
            replyTo = replyTo,
            sessionId = sessionId,
            audioOutputFormat = AudioOutputFormat(codec, sampleRate, channels),
        )
    }
    ...
    private fun rejectAbsent(value: Map<String, Any?>, vararg keys: String) {
        keys.forEach { key ->
            if (value.containsKey(key)) throw ZaraWireException("hello.ok contains unexpected $key")
        }
    }
