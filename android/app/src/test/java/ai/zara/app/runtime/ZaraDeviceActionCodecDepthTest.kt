...
class ZaraDeviceActionCodecDepthTest {
    @Test fun `device action JSON nesting is bounded at sixty four containers`() {
        val boundary = assertThrows(ZaraWireException::class.java) {
            ZaraDeviceActionCodec.decodeServerMessage(server(nestedEnvelope(64)))
        }
        assertFalse(boundary.message.orEmpty().contains("nesting depth"))

        val tooDeep = assertThrows(ZaraWireException::class.java) {
            ZaraDeviceActionCodec.decodeServerMessage(server(nestedEnvelope(65)))
        }
