...
import ai.zara.app.device.AndroidDeviceExecutionReceipt
...
class ZaraDeviceActionCodecTest {
    ...
    fun `decodes strict open_uri action request`() {
        val message = ZaraDeviceActionCodec.decodeServerMessage(
            frames(
                """{"body":{"action_id":"action-1","action_seq":7,"args":{"uri":"https://example.com"},"capability":"open_uri","deadline_ns":999999999999999999,"idempotency":"at_most_once"},"id":"request-1","payload_count":0,"session_id":"session-1","timestamp_ns":4,"trace_id":"trace-1","type":"device.action.request"}"""
            )
        )
    ...
    @Test
    fun `decodes unrestricted android raw action`() {
        val message = ZaraDeviceActionCodec.decodeServerMessage(
            frames(
                """{"body":{"action_id":"raw-1","action_seq":8,"args":{"backend":"root","operation":"shell.exec","arguments":{"command":"id","timeout_ms":"5000"}},"capability":"android_raw","deadline_ns":999999999999999999,"idempotency":"at_most_once"},"id":"request-raw","payload_count":0,"session_id":"session-1","timestamp_ns":4,"type":"device.action.request"}"""
            )
        ) as DeviceServerMessage.Request

        assertEquals(DeviceCapability.AndroidRaw, message.capability)
        assertEquals(8, message.actionSequence)
        assertEquals(
            DeviceActionArguments.AndroidRaw(
                backend = "root",
                operation = "shell.exec",
                arguments = mapOf("command" to "id", "timeout_ms" to "5000"),
            ),
            message.arguments,
        )
    }
    ...
    fun `unknown fields executable args and unsupported capability fail closed`() {
        listOf(
            ...
            """{"body":{"action_id":"a","action_seq":0,"args":{"uri":"https://example.com"},"capability":"open_uri","deadline_ns":9,"idempotency":"at_most_once"},"id":"r","payload_count":0,"session_id":"s","timestamp_ns":1,"type":"device.action.request"}""",
        ).forEach { envelope ->
    ...
    @Test
    fun `completed raw action emits staged backend identity and output`() {
        DeviceActionResultReceipts.stage(
            AndroidDeviceExecutionReceipt("root", "root:0", "uid=0(root)"),
        )
        DeviceActionResultReceipts.bindToAction("raw-1")

        assertEquals(
            """{"body":{"action_id":"raw-1","outcome":"completed","backend":"root","identity":"root:0","output":"uid=0(root)"},"id":"result-raw","payload_count":0,"session_id":"session-1","timestamp_ns":9,"type":"device.action.result"}""",
            envelope(
                ZaraDeviceActionCodec.encodeCompleted(
                    requestId = "result-raw",
                    sessionId = "session-1",
                    actionId = "raw-1",
                    timestampNs = 9,
                )
