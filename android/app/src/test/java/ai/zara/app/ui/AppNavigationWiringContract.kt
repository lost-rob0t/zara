...
object AppNavigationWiringContract {
    ...
    fun settingsSectionsDoNotRemainOneLongForm() {
        ...
        val runtime = settings.substringAfter("AppRoute.Runtime -> {").substringBefore("AppRoute.RemoteApis -> {")
        val remoteApis = settings.substringAfter("AppRoute.RemoteApis -> {").substringBefore("AppRoute.Permissions -> {")
        ...
        check(remoteApis.contains("REMOTE APIS"))
        check(remoteApis.contains("OpenRouter"))
        check(remoteApis.contains("Generic OpenAI-compatible"))
        check(remoteApis.contains("Z.AI Coding Plan"))
        check(remoteApis.contains("Android Keystore"))
        check(remoteApis.contains("onSaveRemoteApi"))
        check(remoteApis.contains("onClearRemoteApiKey"))
