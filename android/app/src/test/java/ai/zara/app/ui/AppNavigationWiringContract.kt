...
object AppNavigationWiringContract {
    ...
    fun settingsSectionsDoNotRemainOneLongForm() {
        ...
        val runtime = settings.substringAfter("AppRoute.Runtime -> {").substringBefore("AppRoute.ModelApis -> {")
        val modelApis = settings.substringAfter("AppRoute.ModelApis -> {").substringBefore("AppRoute.Permissions -> {")
        ...
        check(modelApis.contains("MODEL SERVER"))
        check(modelApis.contains("CloudModelProvider.entries"))
        check(modelApis.contains("Save model API"))
        check(modelApis.contains("Discover models"))
        check(modelApis.contains("Local mode and pure-symbolic mode never send prompts here"))
