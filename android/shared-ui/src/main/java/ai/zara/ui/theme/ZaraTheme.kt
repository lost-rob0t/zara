...
private val StarIntelTokens = OutrunTokens.copy(
...
private val MidnightTokens = OutrunTokens.copy(
...
private val TerminalTokens = OutrunTokens.copy(
...
private val LightTokens = OutrunTokens.copy(
...
/**
 * OLED themes deliberately preserve the accent/semantic identity of their parent
 * while replacing large-area surfaces with true or near black. This mirrors an
 * Emacs derived-theme model: inherit the semantic palette, override only the
 * faces/tokens needed by the derivative.
 */
private fun oled(tokens: ZaraSemanticTokens) = tokens.copy(
    background = Color.Black,
    surface = Color(0xFF010101),
    surfaceElevated = Color(0xFF050505),
    surfaceInput = Color(0xFF020202),
    ambientGlow = Color.Transparent,
)

enum class ZaraTheme {
    Outrun,
    OutrunOled,
    StarIntel,
    StarIntelOled,
...
fun themeTokens(
    ...
    val tokens = when (resolved) {
        ZaraTheme.Outrun -> OutrunTokens
        ZaraTheme.OutrunOled -> oled(OutrunTokens)
        ZaraTheme.StarIntel -> StarIntelTokens
        ZaraTheme.StarIntelOled -> oled(StarIntelTokens)
        ZaraTheme.Midnight -> MidnightTokens
        ZaraTheme.MidnightOled -> oled(MidnightTokens)
        ZaraTheme.Terminal -> TerminalTokens
        ZaraTheme.TerminalOled -> oled(TerminalTokens)
        ZaraTheme.Light -> LightTokens
        ZaraTheme.System -> error("System theme must resolve before token selection")
    }
