...
@pytest.mark.asyncio
async def test_stop_times_out_blocking_composed_invocation_and_fences_late_result(tmp_path):
    _write_plugin(tmp_path / "consumer.py", name="consumer")
    _write_plugin(tmp_path / "provider.py", name="provider", tool_name="provider.read")
    registry = ToolRegistry()
    invocation_started = threading.Event()
    ...
    try:
        ...
        release_invocation.set()
        await stop_task
        pytest.fail("plugin manager stop exceeded the configured lifecycle deadline")

    diagnostics = manager.diagnostics()
    assert any(
        item.state is PluginState.FAILED
        and "active capability invocations" in item.error
        for item in diagnostics
        if item.name in {"consumer", "provider"}
    )
