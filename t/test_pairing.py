...
def _send_request(
    ...
    token: str,
    public_key: str = CLIENT_KEY,
    client_kind: str = "android",
):
...
def test_device_id_and_confirmation_code_are_stable_and_key_bound() -> None:
    ...
    assert derive_device_id(CLIENT_KEY, "desktop") == "desktop-e26d2da3ab58"
...
def test_desktop_pair_client_installs_private_identity_and_server_profile(tmp_path) -> None:
    store = ClientEnrollmentStore(tmp_path / "client-curve.json")
    broker = PairingBroker(
        endpoint="tcp://127.0.0.1:7731",
        server_public_key=SERVER_KEY,
        token="desktop-pairing-token",
        expires_at=int(time.time()) + 30,
        bind_host="127.0.0.1",
    )
    uri = broker.pairing_uri(advertise_host="127.0.0.1")
    result: list[object] = []
    progress: list[object] = []

    def client() -> None:
        try:
            result.append(pair_client(uri, store=store, on_progress=progress.append))
    ...
    try:
        claim = broker.wait_for_claim()
        assert claim.device_id.startswith("desktop-")
        broker.approve(claim)
        thread.join(timeout=2.0)
    finally:
        broker.close()
    ...
    assert progress
    profile = store.ready_profile()
    assert profile is not None
    assert profile.endpoint == "tcp://127.0.0.1:7731"
    assert profile.server_public_key == SERVER_KEY
    assert profile.curve_config().server_public_key == SERVER_KEY
    assert (tmp_path / "client-curve.json").stat().st_mode & 0o077 == 0
