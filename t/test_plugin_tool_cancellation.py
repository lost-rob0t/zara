...
@pytest.mark.asyncio
async def test_unloaded_tool_never_enters_cancellation_scope():
    called = False

    @tool("unloaded_effect")
    def unloaded_effect() -> str:
        """Must never execute after plugin unload."""
        nonlocal called
        called = True
        assert current_tool_cancellation() is not None
        return "unexpected"
    ...
    assert result["messages"][0].status == "error"
    assert current_tool_cancellation() is None
    failed = [event for event in published if isinstance(event, events.ToolFailed)]
    assert len(failed) == 1
    assert failed[0].reason == "tool is no longer available"
