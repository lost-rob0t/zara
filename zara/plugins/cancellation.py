...
def _is_async_only(tool: BaseTool) -> bool:
    ...
    if coroutine is not None and function is None:
        return True
    return tool.__class__._run is BaseTool._run and tool.__class__._arun is not BaseTool._arun
...
class _CancellationBoundTool(BaseTool):
    ...
    async def ainvoke(
        ...
        **kwargs: Any,
        ...
        def execute_sync() -> Any:
            with _bound_cancellation(cancellation):
                return self._wrapped.invoke(input, config, **kwargs)

        async def execute() -> Any:
            if _is_async_only(self._wrapped):
                with _bound_cancellation(cancellation):
