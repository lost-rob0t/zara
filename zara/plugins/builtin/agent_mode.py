...
class AgentModeStore:
    ...
    def claim_due(self, now: Optional[float] = None) -> list[RecurringTask]:
        ...
        Advancing first prevents a failed dispatch from creating a tight retry storm.
        """
        current = time.time() if now is None else float(now)
...
class AgentModePlugin(ServicePlugin):
    ...
    def tools(self):
        def schedule_recurring_task(
            ...
            return (
                ...
                f"runs every {every_minutes:g} minute(s)."
            )
        ...
        return (
            StructuredTool.from_function(
                ...
                description="Schedule a persistent task for Zara to execute repeatedly in the background.",
            ...
            StructuredTool.from_function(
                ...
                name="speak",
                description=(
                    "Speak text aloud with Zara's configured TTS provider. "
                    "This never opens, closes, pauses, or takes ownership of microphone capture."
                ),
            ...
            StructuredTool.from_function(
                ...
                description="Show whether autonomous agent-mode features are active and how many recurring tasks exist.",
            ),
        )
    ...
    def _question_prompt(self) -> str:
        ...
        return (
            ...
            "natural question that would help you learn a useful preference, clarify an active "
            "project, discover something worth remembering, or help with their current goals. "
            "Use memory only if useful. Do not explain why you are asking. Return only the question."
        )
    ...
    def _speak_text(self, text: str) -> str:
        ...
        with self._speech_lock:
            ...
            if not result.success:
                return f"TTS failed: {result.error or 'unknown synthesis error'}"
            ...
            try:
                ...
                if completed.returncode != 0:
                    ...
                    return f"TTS playback failed: {detail or completed.returncode}"
                return "Spoken."
            except subprocess.TimeoutExpired:
                return "TTS playback timed out."
            finally:
                if path:
                    try:
                        os.unlink(path)
