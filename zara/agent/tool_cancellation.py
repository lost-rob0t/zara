...
_BOUND_CANCELLATION_SIGNAL: ContextVar[Optional["_CancellationSignal"]] = ContextVar(
    "zara_bound_tool_cancellation_signal",
    default=None,
)
...
def _new_tool_cancellation_signal() -> _CancellationSignal:
    return _CancellationSignal()


@contextmanager
def _bind_tool_cancellation_signal(signal: _CancellationSignal):
    """Bind one Core-owned signal for the next cancellation wrapper invocation."""

    token = _BOUND_CANCELLATION_SIGNAL.set(signal)
    try:
        yield
    finally:
        _BOUND_CANCELLATION_SIGNAL.reset(token)


@contextmanager
def _claim_tool_cancellation_signal():
    """Consume a bound Core signal once, otherwise create an invocation-local one."""

    signal = _BOUND_CANCELLATION_SIGNAL.get()
    if signal is None:
        yield _CancellationSignal()
        return

    token = _BOUND_CANCELLATION_SIGNAL.set(None)
    try:
        yield signal
    finally:
        _BOUND_CANCELLATION_SIGNAL.reset(token)
...
class CancellationTransportTool(BaseTool):
    ...
    def invoke(
        ...
        **kwargs: Any,
        ...
        with _claim_tool_cancellation_signal() as signal:
