...
class TranscriptNormalizationConfig:
    ...
    def from_mapping(cls, value: object) -> "TranscriptNormalizationConfig":
        ...
        model = _text(
            "s1_mini.model", raw_s1.get("model", "superwhisper/s1-mini"), 256
        )
        styling = _choice(
            "s1_mini.styling", raw_s1.get("styling", "semi-formal"), _STYLING
        )
        structure = _choice(
            "s1_mini.structure", raw_s1.get("structure", "prose"), _STRUCTURE
        )
        context = _choice(
            "s1_mini.context", raw_s1.get("context", "general"), _CONTEXT
        )
