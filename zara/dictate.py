...
def _select_model_device(model_name: str, device: str) -> str:
    if device == "cuda" and str(model_name).strip().lower().endswith(".bin"):
        return "vulkan"
    return device
...
def main(model_name="small", device="cpu", threads=None, workers=2, stop_phrases=None):
    ...
    try:
        ...
        device = _select_model_device(model_name, _normalize_device(device))
