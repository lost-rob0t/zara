...
Unknown protocol majors fail closed. Duplicate runtime ids make a discovery refresh fail atomically. A refresh that makes the selected runtime missing or unselectable clears the active selection and advances the canonical generation so stale runtime/session publications can be rejected.
