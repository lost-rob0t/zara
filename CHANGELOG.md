...
- Zara Android now embeds the polished shared Code Workbench directly under Workspace while the standalone Zara Code APK hosts the same surface, and trusted Zara-signed sibling apps can call the canonical local Prolog runtime through the bounded signature-protected ZARA-PROLOG/1 Binder API.
