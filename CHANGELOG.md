...
- Notifications now use deterministic Prolog policy with metadata-only cross-device privacy by default, restart-safe dedupe/coalescing, stale-peer routing fences, durable routed-action grants, semantic exactly-once effect identity, and fresh postcondition verification; same-generation replay cannot rewrite source/action authority and the pure-symbolic path makes zero provider/model calls.
