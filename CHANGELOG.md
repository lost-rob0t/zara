...
- Zara now exposes Prolog-owned commerce and Mega Brain preference policy facts: DoorDash is the default commerce provider, external purchases are fixed to canonical interactive confirmation, and operators can enable/disable learned preferences plus tune minimum evidence, maximum patterns, and confidence thresholds through validated user configuration.
