...
- Android Runtime Settings now renders the canonical installed runtime registry with live local-model health, stable runtime identities, and selectable runtime state while keeping Auto/Local/Remote routing controls separate.
...
- Android Runtime Settings now keeps the canonical embedded runtime authoritative when optional discovery reports the same runtime id, preventing duplicate discovery from crashing the settings refresh.
