...
- Desktop pure-symbolic conversations now persist canonical Prolog clarification/follow-up context in Zara's existing conversation projection, so multi-turn symbolic dialogue survives backend restart with providers disabled and zero model/provider calls.
