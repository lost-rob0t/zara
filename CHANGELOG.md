...
- Authenticated ZARA/1 peer nodes can now ask or delegate work through the existing Zara runtime, with session-derived authority, bounded deadlines/output budgets, idempotent request replay, canonical cancellation, generation fencing, and typed safe terminal results/errors; unsupported token/tool/model/cost budgets fail closed instead of being silently ignored.
