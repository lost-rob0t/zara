...
- Android remote sessions now recover on their own when the desktop server restarts: a `handshake_required`/`stale_session` turn failure ends the stale session and triggers the normal bounded re-handshake instead of leaving the app silently fake-connected with every turn failing.
