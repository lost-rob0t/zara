...
- Desktop Settings now mirrors Android's settings hierarchy with Runtime, Connection, Permissions, Appearance, Plugins, Updates, Diagnostics, and About sections, plus search, compact semantic cards, and clear live-vs-restart setting badges while continuing to use the same canonical SettingsDocument/ZaraConfig state.
