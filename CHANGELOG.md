...
- Zara now has a reusable receive-only `ZARA-DONATIONS/1` campaign library with decimal USD goal/raised totals and public crypto-wallet display; Android can explicitly import the same document into app-private storage and shows aggregate/campaign totals and receive addresses under Settings → Runtime.
